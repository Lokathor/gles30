...
compile_error!("Please enable either the 'global_loader' or 'static_loader' feature.");

#[cfg(all(feature="global_loader", feature = "struct_loader"))]
compile_error!("Please enable onyl one loader, not both.");
...
fn main() {
  unsafe {
    ...
    assert_eq!(0, SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_ES as _));
    // make window
    let win = SDL_CreateWindow(
      b"gles30 fermium demo\0".as_ptr().cast(),
      50, 50, 800, 600, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL
    );
    if win.is_null() {
    ...
    #[cfg(feature = "global_loader")] {
      ...
      loop {
        if SDL_PollEvent(&mut event) != 0 && event.common.type_ == SDL_QUIT {
          break
        } else {
